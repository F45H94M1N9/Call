subrek_anying(){
xdg-open https://www.youtube.com/channel/UCF61OnE5_ZG3k9LGg-s0e2A?sub_confirmation=1
sleep 10
}
m="\033[31;1m"
k="\033[1;33m"
...
clear
sleep 0.1
printf "${u}╔═╗╦═╗╔═╗╔╦╗     ${bi}╔═╗╔═╗╗  ╗";printf "\n${u}╚═╗╠═╝╠═╣║║║ ${m}<${h}•${m}> ${bi}║  ╠═╣║  ║";printf "\n${u}╚═╝╩  ╩ ╩╩ ╩     ${bi}╚═╝╩ ╩╚═╝╚═╝ ${k}New";printf "\n${b}══════════════════════════════════";printf "\n${b}=${m}[ ${pu}Author ${m}: ${bi}Sandyto Gautama";printf "\n${b}=${m}[ ${pu}Youtube${m}: ${bi}Fash Gaming Indonesia";printf "\n${b}=${m}[ ${pu}Github ${m}: ${hg}github.com/F45H94M1N9${p}";printf "\n${b}══════════════════════════════════";sleep 0.1;printf $p"\n[${h}+${p}] ${k}Contoh ${m}: ${p}85xxxxxxxxx\n";gaskeun_lah_anying
}
gaskeun_lah_anying(){
	sleep 0.5
